...
def load_data(model, data,inputs_dir):
    ...
    data.load(filename=os.path.join(inputs_dir,'load_zones.csv'),
              ...
              param=(model.overgencost, model.loadsheddingcost),
               #Finalmente indicamos que el indice correspnde al set LOADZONES.
...
def export_results(instance,output_dir,flag_concat,concat_dir):
    ...
    GEN_COMMIT.to_csv(os.path.join(output_dir,'gen_commit.csv'))
    GEN_START_UP.to_csv(os.path.join(output_dir,'gen_start_up.csv'))
    GEN_SHUT_DOWN.to_csv(os.path.join(output_dir,'gen_shut_down.csv'))
    ...
    if flag_concat:
        ...
        GEN_COMMIT.to_csv(os.path.join(concat_dir,'gen_commit.csv'))
        GEN_START_UP.to_csv(os.path.join(concat_dir,'gen_start_up.csv'))
        GEN_SHUT_DOWN.to_csv(os.path.join(concat_dir,'gen_shut_down.csv'))
